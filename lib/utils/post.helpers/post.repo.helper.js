...
exports.resolvePostStateTransitions = resolvePostStateTransitions;
const date_converter_1 = require("../date.converter");
function bulkHelper(docs) {
    const removedImagesSet = []; // collect all images to delete (published or not)
    ...
    for (const { data } of docs) {
        ...
        if (data.status !== "Published")
            continue;
        const monthKey = (0, date_converter_1.getMonthKey)(data.createdAt);
...
function resolvePostStateTransitions(oldData, newData) {
